...
class SctpHandler : public sctp::ISctpHandler
{
  ...
  private:
    ...
    {
        auto w = std::make_unique<NmDUSctp>(NmDUSctp::ASSOCIATION_SETUP);
        w->clientId = clientId;
    ...
    {
        auto w = std::make_unique<NmDUSctp>(NmDUSctp::ASSOCIATION_SHUTDOWN);
        w->clientId = clientId;
        sctpTask->push(std::move(w));
    }
    ...
    void onMessage(const uint8_t *buffer, size_t length, uint16_t stream) override
    {
        ...
        auto w = std::make_unique<NmDUSctp>(NmDUSctp::RECEIVE_MESSAGE);
        w->clientId = clientId;
    ...
    {
        auto w = std::make_unique<NmDUSctp>(NmDUSctp::UNHANDLED_NOTIFICATION);
        w->clientId = clientId;
        sctpTask->push(std::move(w));
    }
...
void SctpTask::onLoop()
{
    ...
    case NtsMessageType::DU_SCTP: {
        ...
        switch (w.present)
        {
        case NmDUSctp::CONNECTION_REQUEST: {
        ...
        case NmDUSctp::CONNECTION_CLOSE: {
        ...
        case NmDUSctp::SEND_MESSAGE: {
            receiveSendMessage(w.clientId, w.stream, std::move(w.buffer));
            break;
        }
        case NmDUSctp::UNHANDLED_NOTIFICATION: {
...
void SctpTask::receiveAssociationSetup(int clientId, int associationId, int inStreams, int outStreams)
{
    ...
    auto msg = std::make_unique<NmDUSctp>(NmDUSctp::ASSOCIATION_SETUP);
...
void SctpTask::receiveAssociationShutdown(int clientId)
{
    ...
    // Notify the relevant task
    auto msg = std::make_unique<NmDUSctp>(NmDUSctp::ASSOCIATION_SHUTDOWN);
    msg->clientId = clientId;
    entry->associatedTask->push(std::move(msg));
}
...
void SctpTask::receiveClientReceive(int clientId, uint16_t stream, UniqueBuffer &&buffer)
{
    ...
    // Notify the relevant task
    auto msg = std::make_unique<NmDUSctp>(NmDUSctp::RECEIVE_MESSAGE);
    msg->clientId = clientId;
    msg->stream = stream;
