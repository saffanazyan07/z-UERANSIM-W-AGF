...
static OrderedMap<std::string, CmdEntry> g_DUCmdEntries = {
...
static std::unique_ptr<DUCliCommand> DUCliParseImpl(const std::string &subCmd, const opt::OptionsResult &options,
                                                    std::string &error)
{
    if (subCmd == "info")
    {
        return std::make_unique<DUCliCommand>(DUCliCommand::INFO);
    }
    if (subCmd == "status")
    {
        return std::make_unique<DUCliCommand>(DUCliCommand::STATUS);
    }
                                                ...
                                                std::string &output)
{
    ...
    if (options.has_value())
        return DUCliParseImpl(subCmd, *options, error);
    return nullptr;
}
