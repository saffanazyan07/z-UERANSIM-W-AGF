...
void NgapTask::receiveInitialContextSetup(int amfId, ASN_NGAP_InitialContextSetupRequest *msg)
{
    ...
    auto w = std::make_unique<NmCUNgapToGtp>(NmCUNgapToGtp::UE_CONTEXT_UPDATE);
...
void NgapTask::receiveContextRelease(int amfId, ASN_NGAP_UEContextReleaseCommand *msg)
{
    ...
    // Notify RRC task
    auto w1 = std::make_unique<NmCUNgapToRrc>(NmCUNgapToRrc::AN_RELEASE);
    w1->ueId = ue->ctxId;
    m_base->rrcTask->push(std::move(w1));

    // Notify GTP task
    auto w2 = std::make_unique<NmCUNgapToGtp>(NmCUNgapToGtp::UE_CONTEXT_RELEASE);
    w2->ueId = ue->ctxId;
